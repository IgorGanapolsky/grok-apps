...
async function run() {
    ...
    try {
        ...
        const settingsUrl = `https://play.google.com/console/u/0/developers/${DEV_ID}/app/${APP_ID}/store-settings`;
        console.log(`[promote-bot] Navigating directly to Store Settings: ${settingsUrl}`);
        ...
        for (const line of lines) {
            const l = line.trim();
            if (l.length > 0 && (l.includes("App category") || l.includes("Category") || l.includes("App or game") || l.includes("Finance") || l.includes("Productivity"))) {
                console.log(`  > ${l}`);
            }
        }
        console.log("-----------------------------------\n");

        writeFileSync(resolve(rootDir, "store_settings_text.txt"), pageText);

        const screenshotPath = resolve(rootDir, "store_settings_inspect.png");
