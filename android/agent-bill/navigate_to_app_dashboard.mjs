...
async function run() {
  ...
  ws.onopen = async () => {
    try {
      ...
      console.log(`Navigating back to app dashboard: ${dashboardUrl}`);
      ...
      // Save page text
      writeFileSync("/Users/igorganapolsky/workspace/git/igor/grok-apps/android/Agent-Bill/app_dashboard_restored_text.txt", pageText);

      // Capture screenshot
      const screenshotResult = await sendCommand("Page.captureScreenshot", { format: "png" });
      const buffer = Buffer.from(screenshotResult.data, "base64");
      writeFileSync("/Users/igorganapolsky/workspace/git/igor/grok-apps/android/Agent-Bill/app_dashboard_restored.png", buffer);
